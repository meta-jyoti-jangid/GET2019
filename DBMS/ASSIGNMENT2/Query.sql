...
SELECT  category_name from category where category_id NOT IN (SELECT DISTINCT parent_id from category)
...
SELECT orders.order_id AS "Order Id",
       ...
       orders.order_date AS "Order Date" 
       FROM orders where orders.order_date < DATE_SUB(NOW(), INTERVAL 10 DAY) 
       AND orders.order_id 
