...
/**
 * Counselling class process the counselling method and allocate the program to
 * the student
 * 
 * @author jyoti
 */
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < student.getPrefrence().length; i++) {
				...
				for (int j = 0; j < programList.size(); j++) {
					if ((student.getPrefrence()[i].equals(programList.get(j)
							.getName()) && programList.get(j).getCapacity() > 0)) {
						allocated.add(new AllocatedStudent(student.getName(),
								student.getPrefrence()[i]));
