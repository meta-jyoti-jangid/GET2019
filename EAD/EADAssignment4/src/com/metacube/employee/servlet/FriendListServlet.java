...
import Dao.RetrieveEmployeeData;
import POJO.Employee;
import POJO.Friends;

/**
 * FriendListServlet collects all the friends data in a list and forwards it to
 * the friends page
 * 
 * @author jyoti
 */
public class FriendListServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) {
		...
		try {
			...
			listOfAllData = (List<Object>) session
					.getAttribute("listOfAllData");
			Employee e = (Employee) listOfAllData.get(0);
			RetrieveEmployeeData red = new RetrieveEmployeeData();
			listOfAllFriendsData = red.collectAllEmployeeFriendsData(
					e.getEmpOrganization(), id);
			if (listOfAllFriendsData != null) {
				session.setAttribute("listOfAllFriendsData",
						listOfAllFriendsData);
